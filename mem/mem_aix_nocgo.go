	...
	for _, line := range strings.Split(output, "\n") {
		if parseMemory && strings.HasPrefix(line, "memory") {
			...
			if len(p) > 5 {
				...
				// Derive Used from Total - Free rather than svmon's "inuse"
				// column. svmon does not capture size/inuse/free atomically,
				// so inuse can differ from (size - free) by a page or two,
				// which would otherwise leave Total != Used + Free.
				if vmem.Total >= vmem.Free {
					vmem.Used = vmem.Total - vmem.Free
					if vmem.Total > 0 {
