...
func (p *Process) MemoryMapsWithContext(ctx context.Context, grouped bool) (*[]MemoryMapsStat, error) {
	...
	maps := p.parseMemoryMaps(string(output))

	if grouped {
		// Aggregate every region into a single summed entry, matching the
		// cross-platform grouped contract (one entry, empty Path).
		group := MemoryMapsStat{}
		for _, m := range *maps {
			group.Size += m.Size
			group.Rss += m.Rss
		}
		return &[]MemoryMapsStat{group}, nil
	}
...
// Get num_fds from /proc/(pid)/fd and resolve open-file paths via procfiles.
//
// On AIX, /proc/<pid>/fd entries are inode references (character-special or
// FIFO nodes), not symlinks as on Linux, so os.Readlink cannot map a descriptor
// to a path. The count still comes from the directory listing; the paths are
// resolved with the procfiles command.
func (p *Process) fillFromfdWithContext(ctx context.Context) (int32, []*OpenFilesStat, error) {
	...
	openfiles, err := p.openFilesFromProcfiles(ctx)
	if err != nil {
		return numFDs, nil, err
	}

	return numFDs, openfiles, nil
}

// openFilesFromProcfiles parses `procfiles -n <pid>` to map open file
// descriptors to their paths. Each descriptor is reported as a header line
// ("<fd>: S_IF...") followed by a detail line carrying "name:<path>". Only
// descriptors whose name resolves (regular files, directories) are returned;
// pipes, sockets and terminals report "Cannot be retrieved" and are skipped.
func (p *Process) openFilesFromProcfiles(ctx context.Context) ([]*OpenFilesStat, error) {
	//nolint:gosec // Process ID from internal tracking, not untrusted input
	cmd := exec.CommandContext(ctx, "procfiles", "-n", strconv.Itoa(int(p.Pid)))
	output, err := cmd.Output()
	if err != nil {
		return nil, err
	}
	...
	for _, line := range strings.Split(string(output), "\n") {
		...
		// Descriptor header, e.g. "7: S_IFREG mode:0444 dev:10,4 ...".
		if idx := strings.Index(trimmed, ":"); idx > 0 {
			if fd, perr := strconv.ParseUint(trimmed[:idx], 10, 64); perr == nil &&
				strings.Contains(trimmed[idx:], "S_IF") {
				curFd = fd
				haveFd = true
				continue
			}
		...
		// Detail line carrying "name:<path>".
		if haveFd {
			if nIdx := strings.Index(trimmed, "name:"); nIdx >= 0 {
				name := strings.TrimSpace(trimmed[nIdx+len("name:"):])
				haveFd = false
				if name == "" || strings.Contains(name, "Cannot be retrieved") {
					continue
				}
				openfiles = append(openfiles, &OpenFilesStat{
